...
const AddressMuseum = ({ address }) => {
  ...
  useEffect(() => {
    const loadNaverMapScript = async () => {
      ...
      script.src = `https://openapi.map.naver.com/openapi/v3/maps.js?ncpClientId=YOUR_NCP_CLIENT_ID&submodules=geocoder&r=${Math.random()}`;
      script.async = true;

      // Promise를 사용하여 스크립트 로딩이 완료될 때까지 대기
      await new Promise((resolve, reject) => {
        script.onload = resolve;
        script.onerror = () =>
          reject(
            new Error("네이버 지도 API 스크립트 로딩 중 오류가 발생했습니다.")
          );
      });

      // 스크립트 로딩이 완료된 후에 함수 호출
